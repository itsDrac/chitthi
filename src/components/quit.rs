...
impl Quit {
    ...
    pub fn draw(&self, frame: &mut Frame) {
        ...
        let ok_block = Block::default().style(Style::default().bg(if self.which == 0 {
            Color::Cyan
        } else {
            Color::Black
        }));
        let cancel_block = Block::default().style(Style::default().bg(if self.which == 1 {
            Color::Cyan
        } else {
            Color::Black
        }));
...
fn get_chunks(r: Rect) -> Rc<[Rect]> {
    let popup_chunks = Layout::default()
        ...
        .margin(1)
        .constraints([Constraint::Fill(3), Constraint::Min(1)])
        .split(r);
    popup_chunks
}
